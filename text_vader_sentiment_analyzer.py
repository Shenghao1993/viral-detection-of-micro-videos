...
class SentimentIntensityAnalyzer(object):
    ...
    """
    def __init__(self, lexicon_file="lexicons/sentiment_lexicon.txt"):
        ...
        emoji_lex_dic = {}
        with io.open(os.path.join(dirname(_this_module_file_path_), 'lexicons/emoji_lexicon.txt'), encoding='utf-16') as f:
...
def exec_main():    
    ...
    print("Loading data...")
    with open(os.path.join(feature_dir, 'treated_text.txt'), 'r') as f:
    ...
    for instance in x:
        # senti_scores.append(classifier.polarity_scores(instance)["compound"])
        ...
        senti_scores.append([neg, neu, pos])

    fout = open(os.path.join(feature_dir, 'senti_scores.txt'), 'w')
    for score in senti_scores:
        senti = '\t'.join(str(senti) for senti in score)
        fout.write('%s\n' %senti)
    fout.close()
